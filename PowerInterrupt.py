...
# This is highly redundant.. Because it's only used in like... Three tests.... But I have issues with symmetry.
...
class PowerInterruptTest(QWidget):
    ...
    def __init__(self, instrument_worker: InstrumentWorker | None = None, tabs: QTabWidget | None = None, parent=None):
        ...
        self.powerinterrupt_passed = [False]
        self.powerinterrupt_failed = [False]
    ...
    def updatePowerInterruptStep(self):
        ...
        if self.powerinterrupt_completed == True or self.current_powerinterrupt_step > 0:
            self.powerinterruptlabel.setText(
    ...
    def PowerInterruptResults(self):
        print("Printing PowerInterrupt Test Results")
        ...
        try:
            ...
            if r.status_code == 404:
                QMessageBox.information(self, "No Results", "PowerInterrupt has not been run yet.")
                return
        ...
        step_labels = {
            "step1_powerinterrupt_brewindicator": "Brew Indicator Light During Power Interrupt Test:",
        }

        ordered_keys = [
            "step1_powerinterrupt_brewindicator",
        ]
        ...
        col_width = 68
        report_lines = []
        report_lines.append("Power Interrupt Test")
        ...
        # ---------- Build display ----------
        text = "<h2>Power Interrupt Results</h2>"
        ...
        # ---------- Show window ----------
        win = QMainWindow(self)
        win.setWindowTitle("Power Interrupt Results")
        win.resize(750, 520)
        ...
        self._powerinterrupt_results_window = win
    ...
    def SubtestsCompleted(self):
        ...
        try:
            ...
            if status in ("PASS", "FAIL", "COMPLETED"):
                ...
                self.current_powerinterrupt_step = 2
                print(f"Current step: {self.current_powerinterrupt_step}")
                self.updatePowerInterruptStep()

            elif status == "INCOMPLETE":
                steps = payload.get("data", {}).get("steps", {})
                # Resume at "next" step index
                self.current_powerinterrupt_step = len(steps)
                print(f"Current step: {self.current_powerinterrupt_step}")
                self.updatePowerInterruptStep()
                return
            ...
            print(f"[PowerInterrupt] sync_completed_from_db failed: {e}")
